...
(function ($) {
	...
	$.fn.themewichHovers = function() {
		...
		function hover_overlay_images() {
		 ...
		 jQuery('a img').not('.single-slideshow a img, .featured-image img, .ag_recentprojects_widget a img, .gallery a img, a img.fullimg, a img.fixedimg').each(function() {
			jQuery(this).hover( function() {
	...
	$.fn.themewichSticky = function() {
		...
		/**
		 * Remove height from nav as it's scrolled
		 */
		...
		if ($('body').hasClass('sticky-nav-enabled')) {
			var sticky_nav 		= $('.regular-nav .top-nav'), // regular nav selector
				...
				mobilelink 		= $('.mobile-link'); // mobile link
			...
			if (!sticky_nav.hasClass('stickyloaded')  && sticky_nav.length) {
				var sticky_height 		= sticky_nav.outerHeight(), // height with padding
					sticky_height_np 	= sticky_nav.height(), // height without padding
					minHeight 			= sticky_height - ((sticky_height - sticky_height_np) / 2); // half the padding
				...
				sticky_nav.css({
					...
					'height' : sticky_height + 'px', // set static height
					'min-height': minHeight + 'px' // half the padding
				});
	...
	$.fn.themewichStickySocial = function() {
		...
		var social 			= $(this), // sticky icons
			mobilelink 		= $('.mobile-link'); // if it's a mobile layout
